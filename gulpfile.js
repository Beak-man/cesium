...
function buildPlanetaryCesiumViewer() {
    ...
    /*
        promise = promise.then(function() {
               ...
               var stream = eventStream.merge(
               gulp.src('Source/Workers/**')
                );
        
            return streamToPromise(stream.pipe(gulp.dest("Build/Apps/PlanetaryCesiumViewer/Workers")));
        });
        ...
        promise = promise.then(function() {
            ...
            return streamToPromise(stream.pipe(gulp.dest("Build/Apps/PlanetaryCesiumViewer/Assets")));
        });
        */
     ...
     //   promise = promise.then(function() {
      ...
      //      var stream = eventStream.merge(
      //      gulp.src('Source/Widgets/**/**.css'),
      //       gulp.src('Source/Widgets/*.css')
      //  );
   
      /*  return streamToPromise(stream.pipe(gulp.dest("Build/Apps/PlanetaryCesiumViewer/widgets")));
        });
        
        promise = promise.then(function() {

            var stream = eventStream.merge(
            gulp.src('Source/Widgets/Images/**')

        );
   
          return streamToPromise(stream.pipe(gulp.dest("Build/Apps/PlanetaryCesiumViewer/widgets/Images")));
        });
